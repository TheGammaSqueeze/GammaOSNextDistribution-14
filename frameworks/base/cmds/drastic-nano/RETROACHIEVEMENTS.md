...
Opening the in-game overlay does not freeze the emulator. The emulator continues running behind the menu and the render loop continues ticking, so `rc_client_do_frame` continues executing and achievement evaluation stays live during the pause-screen. When the emulator truly stops (sleep mode, screen off), the render loop stops ticking and `rc_client_do_frame` stops. The client thread then calls `rc_client_idle` at least once per second to keep the player in the active players list and the session alive.
