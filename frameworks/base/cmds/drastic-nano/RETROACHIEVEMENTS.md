...
On single-screen devices, which have no second DS screen to host the detail panel, the Achievements section provides the same information through a controller-driven drill-in rendered on the one screen. Selecting an achievement opens a detail card showing its badge, full multi-line description, unlocked or locked status, point value, and a progress bar (the measured fraction, or complete when unlocked). A "View Leaderboards" entry opens the game's leaderboard list, navigated with the d-pad; selecting a leaderboard opens its online rankings (rank, player name, formatted score), scrollable with the d-pad. Cancel steps back one level (rankings to leaderboards to the achievement list) instead of closing the menu, and switching tabs leaves the drill-in. Which mode is used is chosen automatically per device: dual-panel devices use the bottom-screen panel, while single-panel DRM and SurfaceFlinger single-window devices use the on-screen drill-in.
...
* `OverlayMenu.{h,cpp}`: the Achievements overlay section, the chained on-screen-keyboard login, the rich queued unlock and message banner, the bottom-screen detail panel with achievement details and leaderboard rankings (touch-driven with momentum scrolling and online rankings drill-in), the single-screen controller-driven drill-in (achievement detail with progress bar, leaderboards, and rankings) for devices without a second screen, and the live Hardcore toggle.
